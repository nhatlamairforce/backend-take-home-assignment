...
export const friendshipRequestRouter = router({
  send: procedure
    ...
    .mutation(async ({ ctx, input }) => {
       ...
       */
      const checkIfDeclined = await ctx.db.selectFrom('friendships')
  ...
  accept: procedure
    ...
    .mutation(async ({ ctx, input }) => {
      await ctx.db.transaction().execute(async (t) => {
        ...
        const checkIfFriend = await t.selectFrom('friendships')
        .where('friendships.userId', '=', ctx.session.userId)
        .where('friendships.friendUserId', '=', input.friendUserId)
        .where(
          'friendships.status',
  ...
  decline: procedure
    ...
    .mutation(async ({ ctx, input }) => {
       ...
       */
      return await ctx.db.updateTable('friendships')
